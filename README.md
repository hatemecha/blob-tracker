...
## 2\. Features
  ...
  * **Per-object Colors & Motion Trails:** Each object maintains a unique color and an optional trail showing recent positions.
...
4.  **Control Panel:** A window named "Controls" will appear with sliders to adjust parameters.
      ...
      * **Max missed:** Consecutive frames without assignment before a track is removed.
      ...
      * **Color B/G/R:** Preferred color for new IDs; the "Color" window previews the selection.
...
The script is organized into several functions and classes to modularize the different stages of video processing.
  ...
  * **`Tracker`:** Class that manages the tracking of multiple objects, using the Hungarian algorithm for assignment, optional Kalman filters, per-object colors and trails, and track disappearance logic.
...
### `Tracker`
  ...
  * **`__init__(self, max_dist, use_kalman=False, max_missed=10)`:** Initializes the tracker with the maximum assignment distance, optional Kalman filtering, and the number of consecutive frames an object can miss before being removed. Stores object states, colors, and trails.
  * **`update(self, detections, max_blobs, trail_len, color)`:** Builds a cost matrix between detections and existing objects and uses the Hungarian algorithm for assignment. New objects receive IDs and unique colors; tracks exceeding `max_missed` frames without assignment are discarded. Trails are trimmed to `trail_len`. Returns a list of tracked detections with IDs, colors, and trails.
